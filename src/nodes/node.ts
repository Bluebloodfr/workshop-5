...
export async function node(
  ...
  // State checks and broadcasts the node status
  ...
  // Retrieves the current state of this node
  ...
  // Stops this node from further participation in the consensus
  node.get("/stop", (req, res) => {
    currentNodeState.killed = true;
    res.status(200).send("killed");
  });

  // Handles incoming messages and manages state based on the consensus algorithm
  ...
  // Initiates the consensus algorithm
  ...
  // Launches the server and logs the listening port
